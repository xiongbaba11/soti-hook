...
class SearchService {
    ...
    func search(query: String, token: String, model: String, preferLocal: Bool) async -> SearchResult {
        ...
        do {
            let answer = try await DeepSeekService.shared.ask(question: query, token: token, model: model)
            if answer.isEmpty {
                return .notFound
            }
